 ...
 **    Si une des deux stack n'est pas ordonnees il la swap uniquemment elle
 ** 3. Tant que B n'est pas vide il update costs, senpai, ids. Puis effectue les movements necessaires. (check, move, check move…)
...
void	ft_sort(t_data *data)
{
	...
	if (ft_check_order(data->stack_a) == 0
		...
		ft_swap(data->stack_b, 'b');
	while (data->stack_b->next && ft_set_ids(data->stack_b))
	{
